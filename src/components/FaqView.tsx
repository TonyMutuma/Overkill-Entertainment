      ...
      <section className="max-w-[1280px] mx-auto px-4 sm:px-8 md:px-12 lg:px-16 pb-6">
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          <div className="flex flex-wrap gap-2">
            {categories.map((cat) => (
              <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-2 font-mono text-[11px] uppercase tracking-wider transition-colors cursor-pointer border ${activeCategory === cat ? 'bg-white text-black border-white font-bold' : 'bg-[#0b0f17] text-slate-400 border-slate-700/60 hover:text-white hover:border-slate-700/60'}`}>
                {cat}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          <div className="lg:col-span-8 space-y-3 sm:space-y-4">
            {filteredFaqs.length === 0 ? (
              <div className="bg-[#0b0f17] border-2 border-slate-700/60 p-10 text-center">
              ...
              filteredFaqs.map((faq) => {
                ...
                return (
                  <div key={faq.id} className={`bg-[#0b0f17] border-2 overflow-hidden transition-colors relative ${isOpen ? 'border-blue-500/30' : 'border-slate-700/60'}`}>
                    <img src={DJ_ASSETS.djMixerGear} alt='' className='absolute inset-0 w-full h-full object-cover opacity-[0.06] pointer-events-none' />
          ...
          <div className="lg:col-span-4 space-y-4 sm:space-y-6">
            <div className="bg-[#0b0f17] border-2 border-slate-700/60 p-6 relative overflow-hidden">
              <img src={DJ_ASSETS.festivalStage} alt='' className='absolute inset-0 w-full h-full object-cover opacity-[0.08] pointer-events-none' />
              ...
              <button onClick={handleBookingNavigation} className="w-full py-3 bg-white text-black font-bold text-xs uppercase tracking-wider hover:bg-slate-200 transition-colors cursor-pointer">Check Date Availability</button>
            </div>

            <div className="bg-[#0b0f17] border-2 border-slate-700/60 p-6 relative overflow-hidden">
              <img src={DJ_ASSETS.corporateLounge} alt='' className='absolute inset-0 w-full h-full object-cover opacity-[0.08] pointer-events-none' />
              <div className='absolute inset-0 bg-gradient-to-br from-[#0b0f17] via-[#0b0f17]/85 to-transparent pointer-events-none' />
              ...
              {customQuestionSubmitted ? (
                <div className="p-4 bg-blue-500/10 border border-blue-500/20 text-center">
                ...
                <form onSubmit={handleCustomQuestion} className="space-y-3">
                  ...
                  <button type="submit" className="w-full py-3 bg-blue-500 text-white font-bold text-xs hover:bg-blue-600 transition-colors cursor-pointer flex items-center justify-center gap-1.5 uppercase tracking-wider">
