...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith('/api/youtube-previews')) {
      try {
        if (request.method === 'GET') { const r = await supaFetch(env, 'youtube_previews?order=created_at.desc', {}); if (!r.ok) throw new Error(r.data?.message || JSON.stringify(r.data)); return new Response(JSON.stringify(r.data || []), { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } }); }
        if (request.method === 'POST') { const { url: ytUrl } = await request.json(); if (!ytUrl) return new Response(JSON.stringify({ error: 'url required' }), { status: 400 }); const id = `yt-${Date.now()}`; const r = await supaFetch(env, 'youtube_previews', { method: 'POST', body: JSON.stringify({ id, url: ytUrl }) }); if (!r.ok) throw new Error(r.data?.message || JSON.stringify(r.data)); return new Response(JSON.stringify(r.data?.[0] || { id, url: ytUrl }), { headers: { 'Content-Type': 'application/json' } }); }
        if (request.method === 'PUT') { const id = url.pathname.split('/').pop(); const { url: ytUrl } = await request.json(); const r = await supaFetch(env, `youtube_previews?id=eq.${id}`, { method: 'PATCH', body: JSON.stringify({ url: ytUrl }) }); if (!r.ok) throw new Error(r.data?.message || JSON.stringify(r.data)); return new Response(JSON.stringify({ success: true }), { headers: { 'Content-Type': 'application/json' } }); }
