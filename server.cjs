...
app.get('/api/youtube-previews', async (req, res) => { try { const data = await supaFetch('youtube_previews?order=position.asc&order=created_at.desc'); res.json(data || []); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/youtube-previews', async (req, res) => { try { const b = req.body; const url = b.url; if (!url) return res.status(400).json({ error: 'url required' }); const id = `yt-${Date.now()}`; const position = b.position ?? 0; const size = b.size || 'normal'; const data = await supaFetch('youtube_previews', { method: 'POST', body: JSON.stringify({ id, url, position, size }) }); res.json(data?.[0] || { id, url, position, size }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.put('/api/youtube-previews/:id', async (req, res) => { try { const fields = {}; if (req.body.url !== undefined) fields.url = req.body.url; if (req.body.position !== undefined) fields.position = req.body.position; if (req.body.size !== undefined) fields.size = req.body.size; await supaFetch(`youtube_previews?id=eq.${req.params.id}`, { method: 'PATCH', body: JSON.stringify(fields) }); res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
