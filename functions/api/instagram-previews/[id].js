...
export async function onRequestPut({ request, env, params }) {
  try {
    ...
    if (env.DB) { try { await env.DB.prepare('UPDATE instagram_previews SET url=? WHERE id=?').bind(url, params.id).run(); return new Response(JSON.stringify({ success: true }), { headers: { 'Content-Type': 'application/json' } }); } catch {} }
    const r = await supaFetch2(env, `instagram_previews?id=eq.${params.id}`, { method: 'PATCH', body: JSON.stringify({ url }) });
...
export async function onRequestDelete({ env, params }) {
  try {
    if (env.DB) { try { await env.DB.prepare('DELETE FROM instagram_previews WHERE id=?').bind(params.id).run(); return new Response(JSON.stringify({ success: true }), { headers: { 'Content-Type': 'application/json' } }); } catch {} }
    const r = await supaFetch2(env, `instagram_previews?id=eq.${params.id}`, { method: 'DELETE' });
    if (!r.ok) throw new Error(r.data?.message || JSON.stringify(r.data));
    return new Response(JSON.stringify({ success: true }), { headers: { 'Content-Type': 'application/json' } });
  } catch (e) {
    return new Response(JSON.stringify({ error: e.message }), { status: 500 });
