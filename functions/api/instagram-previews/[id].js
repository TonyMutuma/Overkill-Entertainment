...
export async function onRequestPut({ request, env, params }) {
  try {
    ...
    const r = await supaFetch(env, `instagram_previews?id=eq.${params.id}`, { method: 'PATCH', body: JSON.stringify({ url }) });
...
export async function onRequestDelete({ env, params }) {
  try {
    const r = await supaFetch(env, `instagram_previews?id=eq.${params.id}`, { method: 'DELETE' });
    if (!r.ok) throw new Error(r.data?.message || JSON.stringify(r.data));
    return new Response(JSON.stringify({ success: true }), { headers: { 'Content-Type': 'application/json' } });
  } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500 }); }
}
