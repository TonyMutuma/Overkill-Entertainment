  ...
  try {
    if (env.DB) {
      try {
        ...
        if (results) return new Response(JSON.stringify((results || []).map(mapRow)), { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
      } catch {}
    }
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const submittedAt = b.submittedAt || new Date().toISOString().replace('T',' ').slice(0,19);
    ...
    if (env.DB) {
      try {
        await env.DB.prepare('INSERT INTO booking_inquiries (id, client_name, email, phone, event_type, event_date, venue_name, venue_city, guest_count, selected_package, selected_add_ons, special_requests, estimated_total, submitted_at, status, notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')
          .bind(row.id, row.client_name, row.email, row.phone, row.event_type, row.event_date, row.venue_name, row.venue_city, row.guest_count, row.selected_package, row.selected_add_ons, row.special_requests, row.estimated_total, row.submitted_at, row.status, row.notes).run();
